...
exports.machines_shutdown = function() {
  ...
  for (var i in machines) {
    var m = machines[i];
    console.log("shutting down " +m.VMName );
    if (m.InstanceStatus !== "StoppedDeallocated") {
      cp.execFileSync('node_modules/azure-cli/bin/azure', ["vm", "shutdown", m.VMName ]);
    } else {
      console.log(m.VMName + " already down...");
...
exports.machines_start = function() {
  ...
  for (var i in machines) {
    var m = machines[i];
    console.log("starting up " +m.VMName );
    var p = cp.execFileSync('node_modules/azure-cli/bin/azure', ["vm", "start", m.VMName ], { stdio : null });
    console.log("--- " +p);
  }
}
