...
exports.get_machines = function () {
  var json_data = {};
  var std_err_data = "";
  var p = cp.execFileSync('node_modules/azure-cli/bin/azure', ["vm", "list", "--json" ]);
    json_data = JSON.parse(p);
  return json_data;
}

exports.vm_cmd = function(machines, cmd, status) {
  if (machines.length === 0) {
    return;
  }
  var m = machines.shift();
  console.log("cmd: " + cmd + " " +m.VMName );
  console.log(inspect(m));
  if (m.InstanceStatus !== status) {
    var child = cp.spawn('node_modules/azure-cli/bin/azure', ["vm", cmd, m.VMName ]);
    child.stdout.on('data', function (data) {   process.stdout.write(data.toString());  });
    child.stderr.on('data', function (data) {   process.stdout.write(data.toString());  });
    child.on('close', function (code) {
      console.log("Finished with code " + code);
      exports.vm_cmd(machines, cmd, status);
    })
  } else {
    console.log(m.VMName + " already in status " +  m.InstanceStatus);
    exports.vm_cmd(machines, cmd, status);
  }
}

exports.machines_shutdown = function() {
  var machines = exports.get_machines();
  console.log("machines to_shutdown: " + machines.length);
  exports.vm_cmd(machines, "shutdown", "StoppedDeallocated");
}

exports.machines_start = function() {
  var machines = exports.get_machines();
  console.log("machines_to start: " + machines.length);
  exports.vm_cmd(machines, "start", "ReadyRole");
}
